...
# Calculate PubChem fingerprints using RDKit as a fallback
def calculate_rdkit_fingerprints(smiles, chembl_id):
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        raise ValueError(f"Failed to parse SMILES for {chembl_id}: {smiles}")
    ...
    descriptors = pd.DataFrame([fingerprint_array], columns=columns)
    logging.info(f"Descriptors shape (RDKit): {descriptors.shape}, sample: {descriptors.iloc[0, :5].to_dict()}...")
    return descriptors
...
def calculate_padel_descriptors(smiles, chembl_id, output_csv):
    ...
    if not os.path.exists(smiles_file):
        raise FileNotFoundError(f"SMILES file {smiles_file} was not created.")

    # Default output file for PaDEL-Descriptor
    default_output = "descriptors_output.csv"

    # Command to run PaDEL-Descriptor using padel.sh
    padel_cmd = [
        "sh", "padel.sh",
        "-file", os.path.abspath(smiles_file),  # Use absolute path
        "-2d",
    ...
    try:
        ...
        logging.info(f"PaDEL-Descriptor Output: {result.stdout}")
        if result.stderr:
            logging.warning(f"PaDEL-Descriptor Errors: {result.stderr}")
        # Check if the default output file exists
        if not os.path.exists(default_output):
            raise RuntimeError(f"Default output file {default_output} was not generated by PaDEL-Descriptor.")
        # Rename the default output file to the desired output file
        shutil.move(default_output, output_csv)
    except (subprocess.CalledProcessError, FileNotFoundError, RuntimeError) as e:
        logging.warning(f"PaDEL-Descriptor failed: {str(e)}. Falling back to RDKit.")
        ...
        # Fallback to RDKit
        return calculate_rdkit_fingerprints(smiles, chembl_id)

    # Read the output CSV from PaDEL-Descriptor
    try:
        descriptors = pd.read_csv(output_csv)
        descriptors = descriptors.drop(columns=['Name'])
    except FileNotFoundError as e:
        logging.error(f"Failed to read {output_csv}: {str(e)}")
    ...
    logging.info(f"Descriptors shape (PaDEL): {descriptors.shape}, sample: {descriptors.iloc[0, :5].to_dict()}...")
    return descriptors
