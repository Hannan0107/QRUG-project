...
# Calculate PubChem fingerprints using PaDEL-Descriptor or padelpy
def calculate_padel_descriptors(smiles, chembl_id, output_csv):
    ...
    try:
        ...
        logging.warning(f"PaDEL-Descriptor failed: {str(e)}. Falling back to padelpy.")
        # Fallback to padelpy
        try:
            descriptors_dict = from_smiles(smiles, fingerprints=True, descriptors=False)
            descriptors = pd.DataFrame([list(map(int, descriptors_dict.values()))], columns=descriptors_dict.keys())
        except Exception as e:
            logging.error(f"padelpy failed: {str(e)}")
            raise RuntimeError(f"Failed to compute descriptors using padelpy: {str(e)}")
    else:
